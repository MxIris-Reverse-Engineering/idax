...
* `processorId(): number`
* `processorIdFromRaw(rawId: number): number | null`
* `processor(): number`
* `processorProfile(): { rawId: number, knownId: number | null, name: string, addressBitness: number, bigEndian: boolean, abiName: string | null }`
...
* `currentWidget(): { id: bigint, title: string, type: number } | null` - snapshot
  the active IDA widget; returns `null` in headless sessions.
...
* `demangled(addressOrSymbol: Address | string, form?: 'short'|'long'|'full'): string`
...
* `onSwitchPseudocode(callback: (event: {functionAddress}) => void): Token`
