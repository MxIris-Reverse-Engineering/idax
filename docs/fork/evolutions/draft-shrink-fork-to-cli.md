...
**一处失败与其更正（非本次回归）**：macOS 26.6 的缓存 `open_database` 失败，26.3 及更早
正常。初判为「IDA 9.4 对该版本分片缓存格式的支持上限」，**该判断是错的**。复查发现 26.6
所在目录留有一套未正常关闭的残留数据库（`dyld_shared_cache_arm64e.id0` 等，时间戳早于
本次会话），而每个能正常打开的缓存目录都没有残留。IDA 拒绝打开一个旁边还摊着解包数据库的
输入。此项与本次收缩无关，也不在改动触及的路径上；残留未清理（属用户资料卷，可能承载
未完成的分析）。
