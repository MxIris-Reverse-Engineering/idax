...
## 4) Register a shortcut-only callback

Use `ScopedHotkey` when a command needs only a keyboard shortcut and callback.
The internal action ID is generated by idax, ownership is move-only, and the
action unregisters when the object is destroyed.

```cpp
ida::plugin::ScopedHotkey hotkey;

auto registration = ida::plugin::register_hotkey(
    "Ctrl-Shift-H",
    []() -> ida::Status {
      ida::ui::message("scoped hotkey invoked\n");
      return ida::ok();
    });
if (registration) {
  hotkey = std::move(*registration);
}

// Optional early teardown; destruction is otherwise sufficient.
if (hotkey.active()) {
  (void)hotkey.release();
}
```

Use a named `Action` instead when the same command must appear in a menu,
toolbar, or popup, or needs an availability callback. `activate_action()` and
`ScopedHotkey::activate()` provide programmatic UI-host activation; headless
idalib can reject dispatch even when registration is valid.

## 5) Owning Hex-Rays in Plugin Lifetimes
...
- Named actions require explicit `unregister_action()` for deterministic callback
  reclamation; `ScopedHotkey` supplies automatic ownership for shortcut-only use.
