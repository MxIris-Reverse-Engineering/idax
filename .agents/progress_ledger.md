...
- **16.239. Action Handler Ownership Correction Complete**
  - 16.239.1. Created `fix/action-handler-ownership` from clean `master` and reproduced the teardown abort both with an isolated SDK allocator probe and a full Linux IDA 9.3 plugin lifecycle compiled against pinned SDK 9.4. The base `make_shared` path aborts during kernel deletion; ordinary SDK allocation/deletion succeeds (F632; KB 35.285).
  - 16.239.2. Implemented Decision 19.79: owner-null `ADF_OWN_HANDLER` descriptor, `unique_ptr` registration transfer, weak adapter/shared callback-state separation, active-callback accounting, and next-UI-iteration self-unregister deferral. Updated the public lifetime contract and added focused self-unregister/state-reclamation integration coverage.
  - 16.239.3. Exact-SDK compilation and the focused `loader_processor_scenario` pass. A disposable full GUI host probe enters the callback, accepts self-unregister, confirms the action is absent after one event iteration, completes normal unregister, and exits without allocator failure. The complete 41-target local run builds; 35 pass on the installed cross-minor IDA 9.3 runtime, while six unrelated host/runtime tests fail in executables with no linked action symbols. No active-work or roadmap phase remains for this bounded correction.

- **16.240. CI Recovery and Swift PR 6 Review**
  - 16.240.1. Started CI6.1/CI6.2 on default-branch commit `b8b7b2981a526c952c05c5adfb26c7facdb6054a`; PR 6 head is `25b002ade3ed02df9566f432e945d78ec4c32f1b`. Live Integrations 31193639708 and Validation 31193639724 pass. Bindings 31193639518 passes all six Node/Rust rows and fails all three Python rows in the declaration gate; Linux job 92916410223 identifies the stale plugin header fingerprint. The same failure reproduces locally. F633/KB 35.286 records the repeated audit; local regression and replacement GitHub evidence remain active.
  - 16.240.2. Refreshed only the independently audited plugin fingerprint to `447da9111f38c154389181cac3558ec2fd15c97ebde064007c821613b4f074f3`. Comment-stripped parent/current declarations match; the 38-domain/965-symbol manifest, two focused fingerprint/manifest regressions, strict mypy over 42 files, example syntax checks, diff validation, 592-file candidate privacy, and 4,463-object reachable-history privacy pass. The pre-existing local editable extension lacks the script submodule and prevents full pure-suite collection; replacement CI will build a fresh extension and provide authoritative full-suite evidence. No test or audit gate was weakened.
  - 16.240.3. Pushed checksum correction `852ed24f7bd4ffec81b4ba1fb43c9679b4eaa05e`; replacement runs are Integrations 34177321526, Validation 34177321487, and Bindings 34177321483. Integrations and the macOS Python row pass; remaining rows are still executing. CI6.2 is active on the recorded PR head. Independent source audits and executable Swift/C++/C probes establish F634/KB 35.287; 64 Swift tests in 32 suites pass with the native builder and with explicit native-archive linkage on the default builder, while the unmodified default-builder link fails. Review consolidation and final live CI evidence remain active.
  - 16.240.4. Completed CI6.2/P72.1 by preserving PR head on `review/swift-pr-6-original`, creating `rewrite/swift-bindings` from the CI correction, and reproducing ten distinct correctness/build failures. The user expanded the task to a fresh implementation of the complete PR intent under every applicable IDAX requirement; implementation remains active and no parity claim is made. Every subsequent commit must use explicit file staging, `red -m --staged --run <files>`, then push. Decisions 19.80-19.81 establish the core and Swift rewrite boundaries; F635/KB 35.288 records the discovered superseding architecture constraints.

### 16.240.5 Default-branch CI recovery verified (2026-09-08)

- Closed CI6.1 at commit `852ed24f7bd4ffec81b4ba1fb43c9679b4eaa05e`: Bindings CI run `34177321483` succeeded with all nine jobs, Validation Matrix `34177321487` succeeded with all six jobs, and Integrations CI `34177321526` succeeded with all three jobs.
- Complete-log privacy workflows `34177623485`, `34177849970`, and `34178658182` all succeeded. Evidence was read through GitHub's run and job APIs.
- The repaired fingerprint is justified by the independently audited comment-only plugin header change; no declaration exclusion was added. Windows Python completed its distribution and declaration gates; its existing initialized-host boundary remains unchanged.
- Removed completed CI6.1 from active work. Phase 72 implementation continues separately on `rewrite/swift-bindings`.

### 16.241. Fresh Swift Implementation (Phase 72)

- 16.241.1. Implemented the first fresh semantic C++ prerequisites under existing database/decompiler/instruction/plugin domains. Exact-SDK builds and public API compilation pass; new metadata tests cover all eight graph maturities, copied ownership, parent navigation, stack locations, and save/reopen. Branch fixtures pass 29 x86 and 24 ARM64 cases. Existing-language parity and complete native validation remain active under P72.3.
- 16.241.2. Authored Swift runtime/error/resource support, private package transport, value-schema generator, current-domain value APIs, TypeInfo construction/introspection/details, search option overloads, and type rendering adapters. The full current Swift package builds under the default Swift 6.4 engine. Native lifecycle agents continue actual module exports and callback adapters; foundation/runtime agents continue initialized-host and clean-consumer tests. F636-F637/KB 35.289-35.290 record the discovered packaging/session/coverage constraints.
- 16.241.3. Added `bindings/swift/CONTRACT.md` with SW.A1-SW.A7, falsification probes, bounded impacts, units, and complexity. Added a Clang-based declaration/field/overload inventory; unlike most public headers, current `ui.hpp` requires pinned SDK includes for parsing. P72.2/P72.4-P72.7 remain active: generated method count and successful compilation are not full parity or closure.

- 16.241.4. Fresh Swift foundation passes seven default-builder XCTest cases, the initialized IDA 9.4 process-main lifecycle (thread rejection, deferred ARC cleanup, LIFO session invalidation, script/registry/storage/directory, and stale wrappers after close/reopen), and a clean transitive Consumer → Middle → IDAX package using the real runtime. Stable Swift 6.0 remains a configured language baseline; the local compiler is 6.4 and does not establish execution with a 6.0 toolchain. Full parity, remaining domain/runtime tests, live CI, and release closure remain active.

- 16.241.5. Added complete copied microcode DTO transport, owned Function/View/LvarSnapshot operations, semantic comment positions and saved settings, referenced-type collection, and callback-leased ctree navigation/traversal. Source review found F638/KB 35.291; D19.84 adds activity/resource pins and native session dependency tracking. Core compilation passes; integrated Swift/runtime falsification remains active with parallel microcode-filter and typed-data completion.

- 16.241.6. Completed P72.3 across C++ and Node/Rust/Python: enriched the existing owned microcode model without changing established discriminants, added ancestry-preserving ctree statement navigation and semantic x86/ARM64 branch predicates, validated database save-to and named plugin execution, and added opaque dyld-cache inventory/loading. Seven focused native CTests pass; semantic metadata has 13,794 assertions across five functions/eight maturities with positive switch, floating-point, stack, call, and ancestry fields. The dyld test passes 66 assertions, including 4,083 real-cache images plus header/module/section service execution. Node passes the positive lifecycle, all 277 structural checks, and TypeScript; Rust and Python pass equivalent populated lifecycles, Rust all-target check, Python 20 pure checks/strict typing, and the 39-domain/984-symbol manifest. F639-F641/KB 35.292-35.294 and D19.85 record exact scope and limits. Removed P72.3 from active work; P72.7 retains complete regressions, distribution, live cross-platform CI, and release closure.

- 16.241.7. Explicitly staged 75 coherent core/parity/protocol files, committed through `red -m --staged --run` as `6f0d3fc2c6d84ce6a7704538cbab34120e4952c4`, and pushed `rewrite/swift-bindings`. Candidate privacy passes 719 files and reachable-history privacy passes 4,478 objects before staging; the local Node build symlink is excluded. Swift implementation remains uncommitted and active. Nineteen Swift XCTest cases and the full initialized lifecycle/typed-data/microcode/base-resource runtime now pass with the x86 semantic fixture. P72.7 still requires complete declaration mapping, final runtime/package/live CI, and closure.

- 16.241.8. The new Swift decompiler suite passes Function/View/settings, four dynamically dispatched visitor callbacks, 196 navigated children, escaped-view rejection, exact callback errors, printed/maturity subscription self-close/ARC, explicit-session dependency/ARC, and 40 populated graphs. It additionally reproduces a pre-existing address-map error: `treeitems` indices are ctree node identifiers, not pseudocode lines. Core now queries documented native item coordinates and returns valid deduplicated line/address pairs; focused native/Swift reruns are active. A separate direct C++ probe reproduces bitmask enum construction failure because `BTE_BITMASK` is set before the SDK's regular-enum-only constant insertion; that bounded correction is active under P72.7.1.

- 16.241.9. Completed P72.7.1: native semantic metadata/address-coordinate CTest passes and native type_roundtrip passes 415 assertions, including 4/8-byte high-bit masks and copied comments. The full Swift runtime also passes the corrected coordinate membership/bounds and type-construction paths, alongside lifecycle, typed data, microcode, all decompiler/session/event probes, and close/reopen. Removed P72.7.1 from active work. F642-F643 are now supported by positive pinned-SDK/IDA 9.4 regressions; full P72.7 regression/package/CI closure remains active.

- 16.241.10. Explicitly staged the coordinate/bitmask correction and protocol files, committed through `red -m --staged --run` as `4d7d25eeaff6bd35b7a237d58fc92c2b72f291eb`, and immediately pushed `rewrite/swift-bindings`. Candidate privacy passed 740 files and reachable-history privacy passed 4,554 objects.
- 16.241.11. Completed P72.7.2: mapped-byte predicates now use the SDK presence predicate independently of item flags; first/next/previous fixup traversal returns the documented BadAddress sentinel at exhaustion; nested emission infers an empty destination's byte width from its containing operand and rejects inconsistent or non-propagatable shapes before SDK entry. Native data_mutation_safety, fixup_relocation, and semantic metadata CTests pass; the required-decompiler Swift probe passes positive empty-destination/helper emission plus four validation failures with unchanged blocks. F644-F646/KB 35.297-35.299 and D19.87 record primary evidence and limits.
- 16.241.12. The Swift transport audit additionally found success-plus-null optional strings for undo/redo labels, problem descriptions, and processor ABI names. Explicit nullable schema rules now preserve absent versus allocated-empty values; F647/KB 35.300 records the transport contract. Final integrated runtime, actual module dispatch, declaration mapping, packaging, CI, and closure remain active.

- 16.241.13. Explicitly staged and committed the mapped-byte/fixup/nested-width correction through `red -m --staged --run` as `bcdf5248`, then immediately pushed `rewrite/swift-bindings`. Candidate privacy passed 742 files and reachable-history privacy passed 4,565 objects. The complete required-decompiler Swift helper now passes 24 XCTest cases, all domain/runtime checks, 40 populated graphs and a clean transitive consumer.
- 16.241.14. Completed P72.7.3 after actual module loading exposed a pre-existing loader macro initialization-order failure. A deterministic two-translation-unit fixture crashes with the original macro (SIGSEGV) and passes with lazy module construction; the new loader_initialization_order CTest passes and an isolated IDAlib host discovers and loads the Swift loader. Refreshed only the audited Python loader-header digest; public declarations are unchanged. F648/KB 35.301 and D19.88 record the finding. Multi-addon Swift support-library packaging and full host dispatch remain active under P72.6.
- 16.241.15. The declaration audit now inventories 3,586 current C++ declarations, including four address aliases/constants outside the nested address namespace. Actual compiler output contains 3,473 Swift declarations before final constructor/query additions. The mapping audit identifies every overload/field individually and separates pointer/token/iterator/ARC language adaptations. It exposed missing convenience defaults, copied-record constructors and validity queries; those additions and final source-package/live-CI checks remain active.

- 16.241.16. Committed the loader construction-order correction and Python fingerprint audit through explicit staging and `red -m --staged --run` as `6162cdb7`, then immediately pushed `rewrite/swift-bindings`. Candidate privacy passed 786 files and reachable-history privacy passed 4,578 objects. The complete fresh native build and all 46 CTests now pass in 118.07 s.
- 16.241.17. Reworked Swift add-on packaging into one IDAXShared image plus independently hidden per-addon factories/descriptors. Relocating the complete plugins/loaders/procs/lib tree still permits actual SDK plugin execution (argument 4242), loader acceptance/selection, processor decoding (one byte) and disassembly (nop), with no duplicate IDAX class diagnostics. Two-plugin isolation, repeatable CI host verification and final public mapping additions remain active under P72.6/P72.7.
- 16.241.18. The custom-data generation regression passes actual same-slot type/format reuse, stale operations, old-owner close/ARC, capture deallocation, and replacement callback/item identity preservation. The complete required-decompiler helper passes 25 XCTest cases, all runtime domains, 40 graphs and the clean transitive consumer. Public-source audit additionally adds copied-record constructors, semantic operand/graph records, explicit validity queries, and a compiled inventory example. F650-F652/KB 35.303-35.305 and D19.89-D19.90 record the scope; final mappings/packages/live CI remain active.

- 16.241.19. Completed P72.2/P72.4/P72.5/P72.6: the checked contract and actual compiler inventories cover 3,604 C++ declarations across 41 headers (39 domains plus core/error), mapped to 3,529 public Swift symbols with 2,861 direct, 650 adapted and 93 private-bridge records. Each canonical declaration/default/export body has a review fingerprint; each target includes the compiler identity and readable signature. Stable Swift foundation, all current domains, opaque dyld-cache services, examples and actual exports are implemented. The final clean helper passes 25 XCTest cases, all runtime checks, the public inventory client and the clean transitive consumer; the relocated four-add-on host passes independent first/second/first plugin dispatch, loader selection and processor decode/render. Removed these four completed items from active work. P72.7 retains distribution, live minimum/current-toolchain and existing-language CI, privacy, staged review and closure.
- 16.241.20. Final audit fixed Boolean string-list configuration and restored native default arguments/aggregate defaults; generated schema keys are validated rather than silently ignored. The final C++ inventory includes four export macros and 14 public FormBuilder template members. F653-F655/KB 35.306-35.308 record external-native-archive relinking, semantic/default gates, and GUI evidence boundaries. Corrected KB 35.304 complexity to account for linear resource guards and repeated closed-holder scans; no constant-time total guard claim remains. GUI shortcut activation is unknown on this host: IDAlib returns the exact canonical action-dispatch failure, and an isolated full-IDA probe cannot start because installed Qt lacks its offscreen platform plugin. Registration/state/teardown and the independent real SDK module tests pass; no GUI success is inferred.

- 16.241.21. Native TGZ generation passes with 57 archive entries including changed public headers, libidax and CMake exports. Independent Python manifest (39 domains/984 symbols), strict typing (43 files), Node structural tests (277/277), TypeScript and workflow syntax/action-pin tests pass. The exact-index source audit exposed valid internal Rust licence links; F657/KB 35.310 records the bounded resolution and successful edge probes. Final mapping red-team identified F656/KB 35.309, a FormBuilder default-versus-empty-title constructor-state mismatch; added active P72.7.4 for the focused fix and nonmodal regression. Public root/package documentation now exposes the Swift package and declaration-map location.

- 16.241.22. Exact-index source distribution passes all checks and compiles its extracted Swift target (792 source files). The separate native TGZ structure passes, but a full bytewise scan of its 49 regular members detects an identity-bearing compiler path in libidax.a; therefore native artifact privacy is not a pass. F658/KB 35.311 and active P72.7.5 record the local unpublished artifact and bounded diagnosis. Repository source/history privacy remain passing.

- 16.241.23. Completed P72.7.4 with separate native-equivalent default and explicit-title FormBuilder states and canonical pre-display markup validation. A native rebuild and isolated fresh Swift compilation pass all six lifecycle XCTest cases, including both empty construction states and appended controls. Updated constructor and ARC reference-assignment mapping targets; the actual fresh inventory now contains 3,530 public Swift symbols and all 3,604 canonical mappings pass (2,861 direct/650 adapted/93 private bridge). Removed P72.7.4 from active work. Full native/runtime results remain those recorded above; cross-toolchain CI will build the complete final source.

- 16.241.24. The final private-bridge/adaptation review identified one additional public capability restriction: native caller-authored markup with typed form bindings is not represented by generated bound forms plus unbound markup. F659/KB 35.312 records the distinction; reopened P72.5 and added P72.7.6 for the bounded opaque-argument correction. This does not invalidate the recorded compiler inventory or runtime passes, but declaration coverage alone is not full behavioral parity. Initial source push will start cross-platform CI while native artifact privacy and this final capability correction proceed.

- 16.241.25. Explicitly staged 197 named source/protocol/workflow files, committed through red -m --staged --run as 5e5b71347cf4fddc17fecfa5b3571fc202f82dae, and immediately pushed rewrite/swift-bindings. Exact-index source archive inspection and extracted Swift compilation pass; source/history privacy passes 792 files and 4,786 reachable objects after the commit. The initial rewrite starts Validation Matrix 34188766784, Integrations CI 34188766839 and Bindings CI 34188766858 (20 configured jobs). P72.5/P72.7.5/P72.7.6 remain active for the explicitly recorded custom-form capability and native artifact path corrections. The default-branch-only automatic complete-log audit will skip this branch; collect complete per-job logs and run the same scanner locally before closure.

- 16.241.26. Initial-head Swift 6.3 job 101942322324 passes its complete toolchain/native/26-XCTest/runtime/consumer/inventory/source-package/four-addon/relocated-host workflow. Swift 6.0 job 101942322375 fails the C++ inventory comparison before Swift compilation; local Clang 20 reproduces AST-only attribute/instantiation differences. Added F660/KB 35.313 and active P72.7.7 for source-backed normalization and bounded diagnostics. Independently scanned all four intended add-on dylibs and IDAXShared: all five pass binary privacy; the native CPack archive remains the only observed artifact path failure.

- 16.241.27. Completed P72.7.5: compiler-origin SDK paths were isolated to DWARF sections in 38 objects; literal macro probes distinguish file-prefix from debug-only maps. Applied private lexical/resolved source/build/SDK maps to native targets (D19.91), rebuilt all native targets and CPack, and byte-scanned all 49 regular members/57 entries successfully (38,589,688 bytes). Native unit/error_torture/diagnostics_torture/smoke pass 4/4. Removed P72.7.5 from active work. D19.92 records the ongoing opaque custom-markup argument model under P72.7.6; minimum-toolchain inventory normalization remains active under P72.7.7.

- 16.241.28. Completed P72.7.7 locally: Apple Clang and upstream Clang 20 now produce byte-identical 3,604-entry inventories after source attribute reconstruction and declared-template filtering. Reviewed and refreshed exactly 16 previously omitted leading attribute fingerprints; all identities, signatures, parameter defaults and declaration counts are unchanged. Both inventory checkers now emit bounded structural mismatch diagnostics. Removed P72.7.7 from active work; the next live Swift 6.0 job must still build and execute the minimum compiler/runtime gates under P72.7.

- 16.241.29. Explicitly staged 17 named portability/build/protocol files, committed through red -m --staged --run as 27999fd, and immediately pushed rewrite/swift-bindings. Source/index archive audit passes; post-commit privacy passes 792 files and 4,804 reachable objects. New runs are Validation Matrix 34189484822, Integrations CI 34189484851 and Bindings CI 34189484835. Initial-head Validation Matrix and Integrations both pass, with complete local log audits covering 6 jobs/895,855 bytes and 3 jobs/518,865 bytes respectively. The final head requires its own complete matrix/log evidence; custom typed markup remains active.

- 16.241.30. Actual Swift 6.0 on 27999fd passes the normalized inventory but fails native standard-library compilation after SDK stdio poisoning in two translation units. Added F661/KB 35.314 and active P72.7.9 for include ordering without disabling SDK protection. Custom-form grammar review additionally exposes the canonical SDK -1 cancellation contract and unbound NUL-validation inconsistency; F662/KB 35.315 and active P72.7.8 require native/Swift false/no-commit parity and isolated dispatch evidence. Custom X/dynamic-label character arguments also require per-argument storage independent of QMAXPATH; this remains within P72.7.6.

- 16.241.31. Completed P72.7.9 source correction: moved standard headers before the SDK bridge/type implementation include in database_lifecycle.cpp and type.cpp. The native idax target rebuild passes; preprocessing both complete units confirms fwrite/fflush/stdout retain their SDK forbidden-name definitions afterward. No public declaration or runtime behavior changes. Removed P72.7.9 from active work; the next actual minimum-toolchain run supplies platform evidence under P72.7.

- 16.241.32. Explicitly staged seven named include-order/protocol files, committed through red -m --staged --run as fc7ba52, and immediately pushed rewrite/swift-bindings. Candidate/history privacy passes 793 files and 4,812 reachable objects. Pending custom-form/native cancellation source remains outside this commit; actual minimum-toolchain validation continues on the new head.

- 16.241.33. Completed P72.5/P72.7.6/P72.7.8: opaque FormArgument factories and askForm(markup:bindings:) preserve supported caller-authored layouts with native placeholder ordering, growable character storage and transactional outputs. Independent grammar/storage/ordering review closes the demonstrated gaps; 12 focused Swift custom-form/lifecycle tests pass. Native cancellation/NUL correction passes smoke/type_roundtrip/debugger_ui_graph_event (3/3); the UI fixture has 185 passing assertions, including 38 new controlled-dispatch assertions, and three pre-existing GUI availability skips. Python manifest remains 39 domains/984 symbols after the reviewed UI header digest; C++ inventory changes only that header SHA, with all 3,604 declarations unchanged. Actual Swift symbols are now 3,538, and every mapping passes (2,861 direct/650 adapted/93 private bridge). Removed the three completed items from active work. Nested dynamic substitutions within input labels and arbitrary native toolkit/callback pointers remain explicit boundaries, not mapped equivalent capabilities.
- 16.241.34. Actual Swift 6.0 on fc7ba52 completes every required workflow step successfully, including native compilation, XCTest/runtime/example/consumer, exact inventories, extracted source package, four add-on builds and relocated SDK dispatch. This confirms the inventory and include-order corrections under Xcode 16.2; the final custom-form source still requires its own clean helper and final-head CI under P72.7.

- 16.241.35. The final custom-form source passes the complete clean helper: 32/32 XCTest, every runtime domain and 40 populated microcode/decompiler graphs, inventory example, clean transitive consumer, and exact fresh 3,538-symbol inventory. Generated adapters and all 3,604 canonical mappings pass. Both Swift 6.0 and 6.3 jobs on the preceding include-order head officially conclude success. Final source staging/archive inspection and final-head cross-platform/log closure remain under P72.7.

- 16.241.36. Final implementation distribution checks pass: the exact-index 793-file Git source archive is privacy-clean and its extracted Swift target compiles; all four add-ons and shared support are rebuilt from the final source, pass descriptor/class isolation, and dispatch correctly after relocation. All five images pass bytewise and extracted-string privacy scans. No implementation work remains outside the staged candidate; P72.7 retains final-head CI, complete-log privacy and closure records.

- 16.241.37. Explicitly staged 20 named final-form/protocol files, committed through red -m --staged --run as 2efc56f2b82d47aa9c0cadb710b7a7f3b8587699, and immediately pushed rewrite/swift-bindings. Post-commit privacy passes 793 files and 4,833 reachable objects. Final implementation runs are Validation Matrix 34190905249, Integrations CI 34190905299 and Bindings CI 34190905262. Added a consolidated validation report with assumptions, bounded evidence and pending final CI/log gates; no further implementation changes are planned absent an observed failure.

- 16.241.38. Final implementation Validation Matrix and Integrations both conclude success; their complete-log audits pass 6 jobs/896,626 bytes and 3 jobs/518,924 bytes with exact run/head identity, byte-preserving transfers and terminal-step verification. Bindings has 10/11 successful jobs, including actual Swift 6.0 and 6.3; Windows Python remains in its normal build sequence. P72.7 retains that final job, the complete Bindings log audit and documentation closure.

- 16.241.39. Closed P72.7 and Phase 72 on implementation revision 2efc56f2b82d47aa9c0cadb710b7a7f3b8587699: Validation Matrix 34190905249 passes 6/6 jobs, Integrations CI 34190905299 passes 3/3, and Bindings CI 34190905262 passes 11/11, including actual Swift 6.0 and 6.3. Complete-log audits pass 6/896,626, 3/518,924 and 11/5,658,342 job/byte counts respectively, totaling 20 complete logs and 7,073,892 bytes. Added sanitized run/job/hash/completeness metadata and the consolidated validation report; independent documentation review found no blocking coverage or evidence issue. QG1-QG7 pass within the recorded SW.A1-SW.A11 assumptions and host boundaries. Refreshed the lean hub, linked package/contract evidence, reconciled the historical Swift symbol count and recorded final minimum-toolchain evidence. P72.7 is checked and Phase 72 is removed from active work in this closure update. The closure commit changes documentation/evidence only; subsequent CI verifies that pushed revision without requiring another documentation commit.

- 16.241.40. Completed P72.8 at the user's explicit request: opened PR #11, "Add opaque Swift bindings across all IDAX domains" (https://github.com/19h/idax/pull/11), from rewrite/swift-bindings to master as ready for review. Its description records coverage, native/add-on behavior, provenance, assumptions, quality gates and host limits. Creation head 5f63768486b0605649c0b9c2a1628f91215ea9ad passed all 20 jobs in Validation 34192901291, Bindings 34192901361 and Integrations 34192901370, with complete-log privacy passing 20 logs/7,075,106 bytes. This follow-up records PR creation only; executable sources remain unchanged. No merge or change to PR #6 was requested or performed.

- 16.241.41. Completed P72.9: first-attempt push Validation run 34251129659 failed only in Linux unit job 102145537004 while finalizing a 3,907-byte build-log artifact, after all seven selected tests passed. The upload service returned an intermediary HTTP 403. Attempt 2 had already passed on unchanged revision d69dfcae2c0e3418823bc0005cadf8aaa0307ca6 before this investigation. PR runs 34251134307, 34251134141 and 34251134135 passed 6/6, 11/11 and 3/3; the head exposes 40 successful push/PR check runs and PR #11 is mergeable. F663/KB 35.316 distinguish the observed upload failure from an unestablished source defect; the precise intermediary cause remains unknown. This record changes documentation only and retains the upload failure gate.

- 16.241.42. Began P72.10 at the user's explicit request after the Swift rewrite was merged and pulled onto master. The inclusive range starts at 33165f5687618872da136a5fb76f4ffc1998ce44 ("Document Swift PR 6 review findings and Phase 72 rewrite baseline") and ends at 4cf4268e82b17ac4cb57c36d809106fc9c500056: twelve linear commits with no commit signatures. The authorized operation adds exactly "Co-authored-by: Mx-Iris <mxiris@126.com>" and signs using the configured identity. Earlier history and original source trees remain outside the modification scope; a local backup and exact remote-head lease protect the operation.

- 16.241.43. Completed the P72.10 local rewrite and verification: all twelve commits now contain exactly "Co-authored-by: Mx-Iris <mxiris@126.com>" and valid GPG signatures from configured fingerprint 6070C0392B6B8F3A20C69A14DDC207DEF6D7DC80. Verified every corresponding tree, original author identity/date, original message plus the requested trailer, and parent chain; predecessor 852ed24f7bd4ffec81b4ba1fb43c9679b4eaa05e is unchanged. The rewritten range starts at e2f113f9 and ends at 96d80ac60225c36a83cb5507760536c6c6ff929a; the original range is retained locally as backup/master-before-swift-attribution-20260909. The separate signed protocol-record commit also carries the requested trailer. Assumption ATTR.A1: the explicitly identified range is inclusive on master; the exact first subject, predecessor and twelve-commit count are its verification probes. [Medium impact] published commit IDs change; the push is constrained by the observed remote head 4cf4268e82b17ac4cb57c36d809106fc9c500056. Earlier validation links retain their historical SHAs; the verified unchanged implementation trees preserve their scope. No runtime algorithm changed, so no source tests were repeated. P72.10 is checked and its active-work item is removed in this update.
