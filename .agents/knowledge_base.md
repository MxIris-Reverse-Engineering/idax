...
### 35.285. Action Handler Allocation, State, and Reentrant Teardown [F632]

- A successfully registered named action transfers its adapter to the SDK through an owner-null descriptor with `ADF_OWN_HANDLER`. Allocate the adapter with ordinary `new` so inherited `action_handler_t` allocation functions pair with kernel deletion; `make_shared` is invalid because the object resides inside a separately allocated control block.
- Keep callbacks in wrapper-owned shared `ActionState` storage and give the SDK-owned adapter only a weak reference. Each callback locks a strong local reference before dispatch. Successful unregister erases wrapper state after the SDK call, so user captures are reclaimed even when a headless host defers adapter destruction and remain alive through an in-flight callback.
- A callback cannot directly destroy its currently executing handler on the verified full host. Track active callbacks under the action-state mutex and transfer an in-callback unregister request to `execute_ui_requests`; accepted deferral is the public success boundary, and the next UI iteration performs ordinary unregister.
- Assumption A72.1: supported full IDA hosts honor `ADF_OWN_HANDLER` on the owner-null named descriptor and delete an ordinary-`new` adapter at most once. Falsify with an exact supported host that leaks or multiply deletes this descriptor. Dependent result: registered adapter ownership. **Bounded risk [high]:** an allocator mismatch aborts during plugin teardown; the inherited SDK allocator makes allocation/deallocation symmetric.
- Assumption A72.2: accepted UI requests execute after the current action callback returns and own their `ui_request_t` allocation as documented. Falsify with a supported GUI host that executes the request reentrantly or rejects it after returning a nonzero request identifier. Dependent result: self-unregister safety. **Bounded risk [medium]:** a deferred unregister failure cannot be returned synchronously after acceptance; explicit teardown remains required before module unload.

### 35.286. Plugin Header Audit Refresh After Ownership Documentation [F633]

- Primary evidence: default-branch commit `b8b7b2981a526c952c05c5adfb26c7facdb6054a`, `include/ida/plugin.hpp`, `bindings/python/src/native/plugin_bind.cpp`, `bindings/python/src/idax/plugin.pyi`, and failed Python job 92916410223 in run 31193639518. The public delta changes comments only; existing Python action functions forward to the corrected core without declaration changes.
- Assumption CI6.A1: the binding declaration inventory is unchanged by this header delta. Falsification probes: compare parent/current comment-stripped declarations, audit native forwarding and stubs, and rerun the complete Python manifest and pure/type gates. Dependent result: refreshing only the plugin SHA-256 is sufficient to repair the observed CI failure.
- Bounded risk [medium]: documentation-only header edits intentionally invalidate this byte-based gate; blindly regenerating all hashes would conceal unrelated declaration changes. Refresh one independently audited LF-canonicalized digest. Hashing has O(n) time and O(n) memory for n source bytes; there are no physical-unit calculations.

### 35.287. Swift PR 6 Boundary Review [F634]

- Review target: PR 6 head `25b002ade3ed02df9566f432e945d78ec4c32f1b`, with primary evidence in its changed source, existing validators, and focused executable probes. Missing action counts and Python umbrella/domain records are independent CI blockers. The changed existing C microcode serializer zeroes fields still consumed by safe Rust.
- Swift executor callbacks return the inverse of the C ABI status convention and fail to release retained context on cleanup. Processor extension-only methods do not become protocol requirements and therefore cannot dispatch conformer overrides through `any ProcessorModule`.
- Exact pinned SDK `6929db6868a524496eb66e76e4ec6c9d720a0594` and IDA 9.4 fixture probes reproduce lost parent context in 22 navigated block children and missing offsets in seven stack variables. Swift 6.4 default swiftbuild also exposes a case-insensitive `libIDAX.a`/`libidax.a` lookup collision; an explicit native archive path makes the same 64-test suite pass.
- Assumptions CI6.A2-CI6.A4: findings apply to the recorded PR head; the mock executor lifecycle matches the real C shim callback/cleanup convention; archive collision depends on case-insensitive path lookup and generated product search order. Falsification probes: recheck head/diff, compare actual C executor implementation, and compare unmodified/default versus explicit-archive/native-builder test commands. Dependent results: individual review findings only; the passing value-focused tests do not establish full Swift runtime correctness.
- Bounded impacts: callback/serialization correctness and merge-time CI regressions [high]; parent/stack metadata loss, retained captures, and supported-toolchain link ambiguity [medium]. No architecture is changed by this review. Full dyld-cache runtime behavior remains unknown without its specific fixture; review findings do not claim that coverage.

### 35.288. Swift Rewrite Baseline and Supersession Audit [F635]

- Primary sources: Decisions 19.39, 19.47, 19.60-19.63, and 19.79; current `include/ida/idax.hpp`; Python declaration inventory; PR head `25b002ade3ed02df9566f432e945d78ec4c32f1b`. The current graph already owns nested/reference/call operands and releases native objects. Numeric stroff identities were deliberately removed and cannot be restored by copying older binding code.
...
### 35.297. Mapped-Byte Presence [F644]
- Primary evidence: pinned SDK revision `6929db6868a524496eb66e76e4ec6c9d720a0594`, `bytes.hpp` presence/flag documentation, and `data_mutation_safety_test.cpp`. Assumption: `is_mapped()` tests database byte presence independently of initialized value and metadata; falsify with zero-filled, uninitialized, unmapped, data-head, and tail cases. Native regression includes 22 new assertions. [High] incomplete unknown-byte inventories; wrapper predicate work and storage are O(1), with SDK lookup complexity unknown.

### 35.298. Fixup Exhaustion [F645]
- Primary evidence: public `fixup.hpp` traversal contract and native first/next/previous SDK sentinel. Assumption: BADADDR denotes exhaustion on these documented calls; falsify with empty and singleton bidirectional traversal, then restore fixtures. Native fixup_relocation passes. [Medium] premature iterator errors; wrapper conversion is O(1), SDK traversal complexity unknown.

### 35.299. Nested Microcode Result Width [F646]
- Primary evidence: pinned `hexrays.hpp` nested-operand construction and propagatable-opcode helper, plus copied native and Swift graphs. Assumption: containing operand width specifies an Empty destination's discarded result size; falsify with successful copied emission, helper nesting, explicit matching destination, negative/absent/mismatched width, and non-propagatable opcodes. Native semantic and required-decompiler Swift probes pass, preserving blocks on four invalid requests. [High] rejected valid graph replay or native internal exception; recursive translation is O(operand-tree size) time/space with the existing depth bound. Widths are bytes; positive user-defined-type widths are retained.
...
### 35.303. Declaration Audit Scope and Current Module Selection [F650]
- Primary evidence: `address.hpp` namespace placement, Clang JSON byte offsets/declarations, Swift public symbol graphs and build-product directories. Assumption: audited compiler output corresponds to the current source and all header scopes; falsify with outer aliases, overload/name collisions, deleted constructors, default parameters, public aggregate construction, and source-relative symbol provenance. [High] false completeness claims. Inventory/mapping checks traverse declarations and explicit edges; wrapper work is O(declarations + mapped edges), excluding compiler execution.

### 35.304. Database-Bound Microcode Filter Owners [F651]
- Primary evidence: private microcode registration release paths and NativeResource invalidation. Assumption: database invalidation occurs on the owner thread after guarded SDK activity; falsify with retained filters over close/reopen, explicit callback close and last-owner ARC during lifting. The integrated required-decompiler helper passes. [High] a stale callback remains registered in a later database; the owner-thread guard scans up to R holders and draining Q queued finalizers costs O(R + QR), excluding native destruction; database invalidation can cost O(R²) with retained closed holders. Individual holder-state access after the guard is O(1).

### 35.305. Custom-Data Registration Generations [F652]
- Primary evidence: private `data_custom.cpp` registration and canonical SDK unregister/lookup paths; a runtime fixture requires actual same UInt16 slot reuse for both type and format. SW.A9 bounds detection to wrapper-mediated identity transitions and externally observable metadata changes; external same-slot/same-name replacement without a bridge transition has unknown generation. Positive tests reject stale query/relation/render/scan/analyze/size/define/context tokens and preserve replacement callbacks after old-owner close/ARC. [High] unintended replacement unregister/use. Generation validation uses keyed state lookup; expected O(1) lookup and O(live/retired identities) metadata, with allocation failure propagated.

### 35.306. External Native Archive Relinking [F653]
- Primary evidence: SwiftPM build commands, external archive/product timestamps and a controlled archive-touch/rebuild probe. Assumption: CMake and SwiftPM are independent dependency graphs; falsify by modifying native transport and requiring a fresh executable/shared image before runtime checks. [High] validation against stale native behavior. Clean full builds and explicit shared-image relinking make the tested product concrete; build complexity depends on compiler/linker work and is unknown here.

### 35.307. Semantic Fields, Defaults and Complete Declarations [F654]
- Primary evidence: canonical headers, Clang AST/macros and compiled Swift symbol graphs. Assumption SW.A1/SW.A7: every declaration has a reviewed concrete target preserving semantics or recording a language adaptation. Falsify with overload collisions, Bool-as-integer fields, lost defaults, nullable absence, public template methods and header-level exports. This checkpoint covered 3,604 canonical declarations and 3,529 public Swift symbols; the final custom-form implementation has 3,538 Swift symbols (35.312), with direct/adapted/private-bridge counts unchanged at 2,861/650/93. [High] false completeness; inventory/map validation is O(declarations + mapped targets), excluding compiler execution. Runtime string-list configure/read/restore and the complete helper pass.

### 35.308. GUI Shortcut Evidence Boundary [F655]
- Primary evidence: canonical SDK action failure in the IDAlib runtime test and isolated full-IDA platform-plugin startup diagnostics. Assumption: an interactive IDA host with a functioning Qt platform can dispatch registered actions. Falsification requires actual activation and callback observation in that host; current GUI activation result is unknown. [Medium] interactive action dispatch remains host-gated. Only the exact category/code/message/context failure is accepted in the headless test; registration/state/close still require positive evidence.

### 35.309. FormBuilder Constructor States [F656]
- Primary evidence: `ui.hpp` default/title constructors and private `forms.cpp` markup construction. Assumption SW.A1: mapped construction preserves observable empty-state markup. Falsify default, explicit empty-title, ordinary title and subsequent bound-field markup without displaying a modal dialog. [Medium] lost native construction state. The isolated native rebuild/fresh Swift compile passes six lifecycle XCTest cases, including exact default/title/appended-field markup. Modal entry applies the canonical markup validator; no interactive dialog success is inferred.

### 35.310. Internal Source-Archive Links [F657]
- Primary evidence: the two tracked Rust package LICENSE links and staged Git archive members. Assumption: an internal direct link to an audited regular file is a valid source-distribution input. Falsify actual licence resolution, absolute/relative root escape, missing target, chained link and directory target. [Medium] unusable archives or unintended extraction paths. Link lookup is O(1) expected with a member index; copied content costs O(file bytes).

### 35.311. Native Distribution Binary Paths [F658]
- Primary evidence: complete CPack member bytes processed by the repository's sensitive_categories checker. Assumption: archive structure success and source-tree privacy also imply binary artifact privacy; the libidax.a result falsifies that assumption. P72.7.5 isolates compiler-origin objects and will require a rebuilt full-member scan. [High] identifying path disclosure in a native distribution. Current archive remains local; source-only Git archive privacy passes. Scanning costs O(archive bytes) time and member-buffer storage.

- **35.311 validation update:** The path occurs once in each of 38 native object DWARF line-string sections and names the resolved SDK include root; runtime assertion strings are unaffected in these objects. Controlled SDK macro probes additionally show why file-prefix mapping is needed for __FILE__/__builtin_FILE() literals. After the private lexical/resolved root maps, full native rebuild/CPack succeeds and every one of 49 regular members in 57 entries passes bytewise privacy. Focused native unit/error/diagnostics/smoke CTests pass 4/4. All four intended Swift add-on images plus shared support independently pass byte scanning. This evidence covers the tested GNU/Clang-style mapping path, not an untested MSVC mapping facility.

### 35.312. Caller-Authored Bound Form Markup [F659]
- Primary evidence: canonical ui.hpp ask_form template, public UI+Forms.swift and private form slot dispatch. Assumption SW.A1 that generated forms preserve every typed custom-layout use case is false. [Medium] loss of native layout capability; implement opaque arguments preserving supported native binding kinds and transactional output conversion. Nonmodal preparation can validate markup/types/count; actual modal acceptance remains host-dependent and unknown here.

- **35.312 validation update:** The opaque six-kind argument API and custom-markup overload now pass 12 focused Swift lifecycle/form tests plus independent exact-SDK grammar/storage/order review. Growable character buffers preserve native X capacities and long dynamic labels independently of QMAXPATH; interleaved C/c/R/r groups consume arguments at closing markers. Type/count/unsupported-pointer/malformed/ceiling checks occur before native dispatch. Nested dynamic substitutions within input labels remain unsupported because their native ordering was not established by the inspected grammar. The complete declaration map passes with 3,538 actual Swift symbols.

### 35.313. Cross-Compiler Declaration Inventory [F660]
- Primary evidence: Swift 6.0 Actions job 101942322375, local Clang 20/21 JSON AST comparisons, and the same authoritative header bytes. Assumption SW.A1/SW.A2: normalized inventories are stable across supported compiler AST encodings. Falsify complete declaration/default/export fingerprints under both compilers and require exactly one declared template pattern rather than instantiated copies. [High] false CI rejection or accidentally omitted API; normalization remains source-backed and exact coverage checking remains active.
...
- **35.315 validation update:** The isolated actual-SDK dispatcher probe passes all 38 new assertions for -2/-1/0/1, prepared integer/qstring argument order and mutations, accept-only commit, and empty/NUL rejection without dispatch. Targeted native tests pass 3/3; public declarations are unchanged and the Python UI header audit is refreshed. Swift uses the same false/non-commit convention and preserves unexpected values below -1 as SDK failures.

- **35.313-35.315 final implementation validation:** Both actual Swift 6.0/Xcode 16.2 and Swift 6.3/Xcode 26.6 jobs pass on revision 2efc56f2b82d47aa9c0cadb710b7a7f3b8587699 in Bindings CI 34190905262. Each validates the normalized inventory, native compilation, 32 XCTest cases, initialized runtime suites, source-package extraction/compilation, clean consumer and real relocated add-on dispatch. This supplies the previously pending minimum-toolchain evidence for F660/F661. The full final matrix passes 20/20 jobs; complete-log privacy passes 20 logs/7,073,892 bytes. The validation report and sanitized audit metadata are retained under docs/reviews. SW.A1-SW.A11 and the explicit interactive/platform/Appcall boundaries continue to constrain these results.

### 35.316. Recovered Artifact Finalization Failure [F663]
- Primary evidence: https://github.com/19h/idax/actions/runs/34251129659/attempts/1 and job 102145537004. Compilation and all seven selected tests passed; upload transferred 3,907 bytes before FinalizeArtifact failed with an intermediary HTTP 403. Attempt 2 passed with unchanged source revision d69dfcae2c0e3418823bc0005cadf8aaa0307ca6, and the three corresponding PR runs passed on attempt 1.
- Assumption CI.A1: the observed failure is an intermittent artifact-service path failure. Stress test/falsification: compare identical-head attempts and upload steps; recurrent 403 responses require examining authentication/service conditions, while any build/test failure requires separate diagnosis. Dependent result: no source or workflow correction is established by this incident. Exact intermediary cause is unknown. [Medium impact] artifact publication can fail a job after its tests pass; retaining the gate preserves evidence availability. [Low-impact opportunity] inspect the failed step before rebuilding code. No algorithm or runtime API changed; the artifact byte count is exact.
